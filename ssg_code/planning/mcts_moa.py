        ...
        if action not in self.children:
            ...
            for i,player in enumerate(self.player_name):
                ...
                if target == self.drift_num:
                    action_dict[player]=np.random.choice(6,p=all_obs[player]['action_mask']/np.sum(all_obs[player]['action_mask']))
                else:
                    model_id=self.model_id_list[i]
                    action_prob_dist=self.model_list[player].get_action(all_obs[f'player_{model_id+1}'],self.state['time'],self.drift_pos[target])
                    action_dict[player]=np.random.choice(6,p=action_prob_dist)
